...
# renames all the instances of the current PoseBlender with the prompted new name
# searches through files and replaces in there as well

# parent directory of the pose-blender
$TOOLS_DIR = (Get-Item -Path "..\").FullName

# define current tool name variables
$CURRENT_base_case = "pose-blender"
$CURRENT_snake_case = "pose_blender"
$CURRENT_PascalCase = "PoseBlender"
...
# Find instances of pose_blender in files and replace them with the new tool name
...
Foreach-Object {
    ...
    ((Get-Content -path $_.FullName -Raw) -replace "pose-blender", $NEW_base_case) | Set-Content -Path $_.FullName
